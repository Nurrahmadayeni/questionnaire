<?php
	...
	elseif ($act=='addUser') {
		...
		if($_SESSION['level']=='unit' || $_SESSION['level']=='super'){
			...
			if(!isset($_POST['value_style_ans'])){
				$query = "INSERT INTO question (id_survey, question, id_style_ans)
								VALUES('$id_survey','$question' ,'$style')";
				// echo $query;
				$sql_insert = $mysqli->query($query);
				...
				$query = "INSERT INTO question (id_survey, question, id_style_ans, answer_value)
									VALUES('$id_survey','$question' ,'$style', '$value_chosen')";
				// echo $query;
				$sql_insert = $mysqli->query($query);
		...
		$query_delete1 = "DELETE FROM survey WHERE id_survey='$_POST[srv]' and id_owner='$_SESSION[status]'";
		echo "$query_delete1";
		...
		if($level=='unit'){
			...
			foreach ($unit as $key => $value) {
				if($mat_kul=='1'){
					$insert = $mysqli->query("INSERT INTO survey (id_owner, objective, matakuliah, title, start_date, due_date, mhs, thn_ajaran, semester) 
					VALUES('$id_owner', '$value', '1' ,'$title', '$start_date', '$due_date', '$p[thn_ajaran]', $p[semester])");
				}else{
					$insert = $mysqli->query("INSERT INTO survey (id_owner, objective, matakuliah, title, start_date, due_date) 
						VALUES('$id_owner', '$value', '0' ,'$title', '$start_date', '$due_date'");
				}
				$max = $mysqli->query("SELECT max(id_survey) as max FROM survey")->fetch_object()->max;
				foreach ($_POST['sampel'] as $key => $value) {
					$mysqli->query("UPDATE survey set $value=1 WHERE id_survey ='$max'");
        ...
        while($qst = $r_n->fetch_assoc()){
        	$insert_qst = $mysqli->query("INSERT INTO question (id_survey, question, id_style_ans, answer_value)
								VALUES('$max','$qst[question]' ,'$qst[id_style_ans]', '$qst[answer_value]')");
        }

		if($insert_qst){
			echo "Survey Berhasil Ditambah";
		}else{
			echo "Gagal, Silahkan cek kembali data yang diinput";
		}		
	}elseif($act=='getMaxSrv'){
		$max = $mysqli->query("SELECT max(id_survey) as max FROM survey where created_by='$_POST[username]'")->fetch_object()->max;
		echo $max;
		// echo " Query SELECT max(id_survey) as max FROM survey where username='$_POST[username]' ";
		// var_dump($_POST);
	}elseif($act=='jlh_tanya'){
		$id_survey = $mysqli->query("SELECT id_survey FROM survey where created_by='$_SESSION[username]' order by id_survey desc limit 1")->fetch_object()->id_survey;
